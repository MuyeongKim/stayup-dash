...
<p align="center"><sub>샘플 데이터로 실행한 1920×1080 표출 화면 · 실제 인물·출동 정보가 아닙니다.</sub></p>
...
`이륙·착륙` 기록은 대상 기체를, `인원교대` 기록은 대상 인원을 선택한 뒤 입력합니다. 최근 3건 밖의 기록은 `전체 기록 조회 · 정정 · 취소`를 펼쳐 찾습니다. 검색어는 내용·구역·날짜를 대상으로 하며 유형과 취소 여부도 함께 필터링할 수 있습니다.
...
> [!IMPORTANT]
> **성명은 넣지 않는 것을 운용 규칙으로 두십시오.** `성명` 칸에는 실명 대신 `대상` 또는 이니셜만 적습니다. 이 도구에서 성명은 **표출용일 뿐 어떤 판정에도 쓰이지 않으므로 기능을 하나도 잃지 않습니다.** 반면 실명을 넣으면 로그인·암호화·감사로그가 없는 브라우저 저장소와 다운로드 폴더, 그리고 자동 보관 파일에 실종자 실명이 평문으로 남습니다. 동원된 별도 팀은 기관의 개인정보 처리 근거 우산 밖에 있으므로 이 규칙이 특히 중요합니다. 좌표·인상착의·특이사항만으로도 수색에 필요한 정보는 모두 갖춰집니다.
...
이전 v1.2.1의 Chrome 실행 감사에서 확인한 기본 동작은 다음과 같습니다. 2026-09-05 작업본의 재검증 결과와 범위는 아래 `이번 업데이트`에 별도로 적었습니다.
...
2026-09-05 코드리뷰 후 수정보완 작업입니다. 아래 내용은 현재 작업본 기준이며, 앱 버전은 기존 GitHub 자동 버전 정책에 따라 릴리스 시 갱신합니다.
...
회귀검사는 [tests/regression.html](./tests/regression.html)의 기존 130건과 [tests/review-fixes.js](./tests/review-fixes.js)의 신규 46건을 합쳐 **176건**입니다. 내부 함수·합성 입력·브라우저 저장소·계산된 레이아웃을 검사합니다. 실제 브라우저 UI에서 수행한 두 창 동기화와 화면 검토는 별도 확인이며, 물리 장비 인수시험을 대체하지 않습니다. 자세한 실행 방법은 [개발자 메모](#회귀-검사)를 참고하십시오.
...
[tests/regression.html](./tests/regression.html)을 로컬 HTTP로 열면 앱을 iframe에 띄워 내부 함수·합성 입력·저장소·레이아웃 **176건**을 검사합니다. 같은 폴더의 [review-fixes.js](./tests/review-fixes.js)도 필요합니다. 실제 파일 선택 권한, 물리 터치·스피커, 절전 복귀는 별도 인수시험 대상입니다.
